...
from resnet.cifar10.attacker import Attacker
...
@click.option('--attack-type', '-t', type=click.Choice(['pgd', 'fsgm']),
              default='pgd')
...
@click.option('--arch', '-a', type=click.Choice(MODELS.keys()),
              default='resnet20')
@click.pass_context
def attack(ctx, dataset_dir, checkpoint, restore,
           batch_size, attack_type,
    ...
    attacker = Attacker.from_checkpoint(arch, num_classes, 
                                        file_utils.load_latest_checkpoint('./run', arch),
                                        use_cuda=use_cuda)
    
    acc = attacker.run_eval(test_loader)
    print(f"Accuracy after attack: {acc}")
